...
import { decodeOAuthState, exchangeOAuthCode } from "@/lib/integrations/oauth";
...
export async function GET(
  ...
  if (!code || !stateRaw) {
    return redirectToIntegrations(request, { error: "Missing code/state from provider callback." });
  }

  const state = decodeOAuthState(stateRaw);

  if (!state || state.provider !== params.provider) {
    return redirectToIntegrations(request, { error: "Invalid OAuth state." });
  }
  ...
  if (!user) {
    return NextResponse.redirect(new URL("/login", appUrlFromRequest(request)));
  }

  if (user.id !== state.userId) {
    return redirectToIntegrations(request, { error: "OAuth user mismatch." });
  }
  ...
  if (profileError || !profile?.workspace_id) {
    return redirectToIntegrations(request, {
      error: profileError?.message ?? "Missing workspace for current user.",
    });
  }

  if (profile.workspace_id !== state.workspaceId) {
    return redirectToIntegrations(request, { error: "Workspace context mismatch." });
  }
  ...
  try {
    ...
    return redirectToIntegrations(request, { success: `${params.provider}_connected` });
  } catch (error) {
    const message = error instanceof Error ? error.message : "OAuth callback failed.";
    return redirectToIntegrations(request, { error: message });
  }
}